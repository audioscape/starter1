// This version has a bug when adding a new account.  Will delete in June 2017.
...
$(document).ready(function() {
  jQuery(document).ready(function($){
    ...
    function runModernizer(attempts) {
      if (attempts > 100) {
        console.log("Unable to load modernizr.js: " + attempts + " attempts.");
        return;
      }
      if (typeof Modernizr == "undefined") {
        tryModernizerAgain(attempts+1)
        return;
      }
    ...
    function tryModernizerAgain(attempts) {
      setTimeout(function() {
        runModernizer(attempts); //Try again
      }, 100);
    }
  });
  ...
  jQuery.fn.putCursorAtEnd = function() {
    return this.each(function() {
        ...
        function handleSignUp() {
          ...
          firebase.auth().createUserWithEmailAndPassword(email, password).catch(function(error) {
              ...
              if (errorCode == 'auth/weak-password') {
                  ...
                  alert(errorMessage + " errorCode: " + errorCode); // auth/network-request-failed
              }
              console.log(error);
          });

          // TEMP
          window.location = "/defense";
        }
        ...
        function initApp() {
                    ...
                    if(emailVerified == false){
                      if(isSignIn == false) {   //if this is the sign up process
                        ...
                        database.ref('users/' + uid).set({
                          ...
                          duns: duns,
                          domain: domain
                        }).then(function () {   //once the user information is added to the realtime database
                      ...
                      firebase.database().ref('users/' + uid).once('value').then(function(snapshot){
                        ...
                        if (typeof customInfo != "undefined" && customInfo != null) {
                          //alert(objToString(customInfo));
                          if (typeof customInfo.firstName != "undefined") {
            ...
            $(document).ready(function() { 
              if (action == "signin" || action == "signout") { // Trigger for both. Avoids error if user tries to sign-in when already signed in.
                $('.cd-signout').trigger("click"); // Calls firebaseSignout() via quickstart-sign-out
              }
            });
        ...
        window.onload = function() {
            initApp(); // Moved into index.html. Stet, moving didn't work for signin.
        };
